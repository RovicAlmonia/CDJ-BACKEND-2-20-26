    ...
    const data = headers.map((hdr) => {
      const hdrDetails = details
        ...
        .map((d) => ({
          ...
          StartOfFiling: formMap[d.FormCode]?.StartOfFiling || "—",
        }));
...
module.exports.postmonitor = async function (req, res) {
  try {
    ...
    // ── Load form metadata ────────────────────────────────────
    const forms   = await safeSelect({ tableName: "tblbirforms", fields: ["*"] });
    const formMap = Object.fromEntries(forms.map((f) => [f.FormCode, f]));
    ...
    for (const code of formcodes) {
      ...
      const dtl = await insert({
        ...
        fieldValue: {
          ...
          IsFiled:       0,
          Status:        status,
          // StartOfFiling is informational — lives in tblbirforms, not stored in tblmonitordtl
        },
      });
      ...
      dtlResults.push({
        ...
        insertId:      dtl?.id ?? dtl?.insertId ?? dtl?.data?.insertId,
      });
    }
...
module.exports.selectbirforms = async function (req, res) {
  try {
    const data = await safeSelect({ tableName: "tblbirforms", fields: ["*"] });
    res.status(200).json({ success: true, data });
  } catch (err) {
